...
public class FlowInstanceController {
    ...
    public Json<String> start(@RequestBody StartFlowInstanceCommand command) {
        FlowInstance flowInstance = flowInstanceService.start(command.getFlowSpecCode(), command.getContext(), new FlowUser(command.getUserId()));
    ...
    public Json<Boolean> fire(@RequestBody FlowInstanceNodeCommand command) {
        flowInstanceService.fireNode(command.getFlowInstanceId(), command.getNodeId(), command.getContext(), new FlowUser(command.getUserId()));
    ...
    public Json<Boolean> retry(@RequestBody FlowInstanceNodeCommand command) {
        flowInstanceService.retryNode(command.getFlowInstanceId(), command.getNodeId(), command.getContext(), new FlowUser(command.getUserId()));
        return Json.success(Boolean.TRUE);
    }

    @PostMapping("/skip.json")
    public Json<Boolean> skip(@RequestBody FlowInstanceNodeCommand command) {
        flowInstanceService.skipNode(command.getFlowInstanceId(), command.getNodeId(), command.getContext(), new FlowUser(command.getUserId()));
        return Json.success(Boolean.TRUE);
    }

    @PostMapping("/cancel.json")
    public Json<Boolean> cancel(@RequestBody FlowInstanceNodeCommand command) {
        flowInstanceService.cancelNode(command.getFlowInstanceId(), command.getNodeId(), command.getContext(), new FlowUser(command.getUserId()));
        return Json.success(Boolean.TRUE);
    }

    @PostMapping("/terminate.json")
    public Json<Boolean> terminate(@RequestBody FlowInstanceCommand command) {
        flowInstanceService.terminate(command.getFlowInstanceId(), command.getContext(), new FlowUser(command.getUserId()));
        return Json.success(Boolean.TRUE);
    }
